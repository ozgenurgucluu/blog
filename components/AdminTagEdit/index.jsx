...
const AdminTagEdit = ({ tag }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const [reqError, newTag] = await updateTag(
        selectedTag.id,
        selectedTag
    );
  ...
  return (
    <div className="p-4 shadow-sm shadow-slate-200">
      ...
      <div className="flex gap-3 items-center">
        <label className="text-gray-700 font-medium">Arka Plan Rengi Seçin:</label>
        <input
          type="color"
          className="w-12 h-12 p-1 border-2 rounded-md cursor-pointer border-gray-300 bg-gray-100"
          value={selectedTag.bgColor}
          onChange={(e) => setSelectedTag({...selectedTag, bgColor: e.target.value})}
        />
      </div>
      
      <div className="flex gap-3 items-center">
        <label className="text-gray-700 font-medium">Metin Rengi Seçin:</label> {selectedTag.textColor}
        <input
          type="color"
          className="w-12 h-12 p-1 border-2 rounded-md cursor-pointer border-gray-300 bg-gray-100"
          value={selectedTag.textColor}
          onChange={(e)=>setSelectedTag({...selectedTag,textColor:e.target.value})}
        />
      </div>
